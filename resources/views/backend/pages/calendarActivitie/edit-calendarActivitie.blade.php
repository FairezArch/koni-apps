        ...
        <div class="shadow-sm p-3 mb-5 bg-white rounded">
            ...
            <form id="forminput" class="forminput mt-3 mb-3">
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" value="{{ $calendar_activitie->match_name }}" name="name_match" id="name_match"
                            class="form-control">
                    </div>
                </div>
                <div class="form-group row">
                        ...
                        <select name="sports_id" id="sports_id" class="sports_id form-control">
                            @foreach ($sports as $sport)
                                <option value="{{ $sport->id }}"
                                    {{ $sport->id == $calendar_activitie->sports_id ? 'selected' : '' }}>
                                    {{ $sport->sportbranch_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                <div class="form-group row">
                            ...
                            <div class="col-md-6">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" value="{{ $calendar_activitie->date_from }}" name="date_from"
                                            id="date_from" class="form-control">
                                    </div>
                                    <div class="col-md-6">
                                        <input type="time" id="datetime_from" name="datetime_from" value="{{ $calendar_activitie->datetime_from }}">
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="row">
                                    <div class="col-md-6">
                                        ...
                                        <input type="date" value="{{ $calendar_activitie->date_to }}" name="date_to"
                                            id="date_to" class="form-control">
                                    </div>
                                    <div class="col-md-6">
                                        <input type="time" id="datetime_to" name="datetime_to" value="{{ $calendar_activitie->datetime_to }}">
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <textarea name="address" id="address" cols="30" rows="10"
                                class="form-control">{{ $calendar_activitie->address }}</textarea>
                    ...
                    @if (!empty($calendar_activitie->file_event))
                        <div class="form-group row text-center">
                            <div class="col-sm-12">
                                <div class="show-image fade-image-cust d-inline-block" id="show-image"
                                    style="width: 200px; height: auto;">
                                    <img src='{{ asset('storage/event/' . $calendar_activitie->file_event) }}'
...
@section('script-footer')
    <script type="text/javascript">
        ...
        $('#forminput').on('submit', function(e) {
            ...
            formData.append('datetime_to', $('#datetime_to').val());
            formData.append('address', $('#address').val());
            formData.append('file_event', $('#file_event').get(0).files[0]);

            $.ajax({
                url: "{{ route('calendar-activitie.update', $calendar_activitie->id) }}",
                type: "POST",
                processData: false,
                contentType: false,
                data: formData,
