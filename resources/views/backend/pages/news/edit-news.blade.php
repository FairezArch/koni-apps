        ...
        <div class="shadow-sm p-3 mb-5 bg-white rounded">
            ...
            <form id="forminput" class="forminput mt-3 mb-3" enctype="multipart/form-data"
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" value="{{ $tidings->title }}" name="title" id="title" class="form-control" placeholder="Judul berita...">
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="short_content" class="form-control" id="short_content" cols="30" rows="5"
                            class="w-100">{{ $tidings->short_content }}</textarea>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="content_news" class="form-control" id="content_news" cols="30" rows="10"
                            class="w-100">{{ $tidings->content }}</textarea>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        ...
                        <div id="preview">
                            @foreach ( json_decode($tidings->file_news) as $file)
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <div id="list_hastag" class="row">
                            @foreach ( json_decode($tidings->hashtags) as $hashtags )
                                <div class="col-md-3">
                    ...
                    <div class="col-sm-10">
                        <select name="sports_id" id="sports_id" class="form-control">
                            @foreach ($sports as $sport)
                                <option value="{{ $sport->id }}" {{ $sport->id == $tidings->sports_id ? 'selected' : '' }}>{{ $sport->sportbranch_name }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                ...
                                <div class="col-sm-8">
                                    <input type="date" name="date_from_news" id="date_from_news" class="form-control" value="{{ !empty($tidings->showtime_from) ? $tidings->showtime_from : \Carbon\Carbon::today()->format('Y-m-d') }}">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                                ...
                                <div class="col-sm-8">
                                    <select name="category_news_id" id="category_news_id" class="form-control">
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}" {{ $category->id == $tidings->category_news_id ? 'selected' : '' }}>{{ $category->category_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-8">
                                    <select name="status_news" id="status_news" class="form-control" {{(!in_array(3,[1,2])) ? '' : 'disabled'}}>
                                        <option value="3" {{ $tidings->status == 3 ? 'selected' : '' }}>Menunggu Konfirmasi</option>
                                        <option value="2" {{ $tidings->status == 2 ? 'selected' : '' }}>Ditolak</option>
                                        <option value="1" {{ $tidings->status == 1 ? 'selected' : '' }}>Diterima</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-group row d-none" id="reason-form">
                    ...
                    <div class="col-sm-10">
                        <textarea name="reason" id="reason" cols="30" rows="10" class="form-control" {{(!in_array(3,[1,2])) ? '' : 'disabled'}}>{{ $tidings->reason }}</textarea>
                    </div>
                </div>
                @else
                @endhasanyrole
...
@section('script-footer')
    ...
    <script type="text/javascript">
        ...
        $('#forminput').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('tidings.update', $tidings->id) }}",
                type: "POST",
                processData: false,
                contentType: false,
                data: formData,
