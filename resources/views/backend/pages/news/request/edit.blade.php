    ...
    <div class="container-fluid">
        <div class="shadow-sm p-3 mb-5 bg-white rounded">
            ...
            <form id="forminput" class="forminput mt-3 mb-3" enctype="multipart/form-data"
                ...
                <input type="hidden" name="type_news" id="type_news" class="type_news" value="4" />
                <div class="form-group row">
                    <label for="title" class="col-sm-2 col-form-label">Title</label>
                    <div class="col-sm-10">
                        <input type="text" value="{{ $tidings_request->title }}" name="title" id="title" class="form-control" placeholder="Judul berita...">
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="short_content" class="form-control" id="short_content" cols="30" rows="5"
                            class="w-100">{{ $tidings_request->short_content }}</textarea>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <textarea name="content_news" class="form-control" id="content_news" cols="30" rows="10"
                            class="w-100">{{ $tidings_request->content }}</textarea>
                    </div>
                </div>
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        ...
                        <div id="preview">
                            @foreach ( json_decode($tidings_request->file_news) as $file)
                                <img src="{{asset('storage/news/'.$file)}}" alt="{{$file}}" style="margin: 5px 10px" height="100">
                            @endforeach
                        </div>
                    </div>
                </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <div id="list_hastag" class="row">
                            @foreach ( json_decode($tidings_request->hashtags) as $hashtags )
                                <div class="col-md-3">
                                    <p class="max-char-length">{{$hashtags}}</p>
                                </div>
                            @endforeach
                        </div>
                    </div>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        ...
                        <input type="hidden" name="sports_id" id="sports_id" class="form-control" value="{{Auth::user()->sports->id}}">                        
                        <input type="text" name="sport_branch_name" id="sport_branch_name" class="form-control" value="{{Auth::user()->sports->sportbranch_name}}" disabled>                        
                        @else
                        <select name="sports_id" id="sports_id" class="form-control">
                            @foreach ($sports as $sport)
                                <option value="{{ $sport->id }}" {{ $sport->id == $tidings_request->sports_id ? 'selected' : '' }}>{{ $sport->sportbranch_name }}</option>
                            @endforeach
                        </select>
                        @endif
                    </div>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                ...
                                <div class="col-sm-8">
                                    <input type="date" name="date_from_news" id="date_from_news" class="form-control" value="{{ !empty($tidings_request->showtime_from) ? $tidings_request->showtime_from : \Carbon\Carbon::today()->format('Y-m-d') }}">
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                                ...
                                <div class="col-sm-8">
                                    <select name="category_news_id" id="category_news_id" class="form-control">
                                        @foreach ($categories as $category)
                                            <option value="{{ $category->id }}" {{ $category->id == $tidings_request->category_news_id ? 'selected' : '' }}>{{ $category->category_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                ...
                @hasanyrole('superadmin|koni')
                <div class="form-group">
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                <label for="status_news" class="col-sm-4 col-form-label">Status</label>
                                <div class="col-sm-8">
                                    <select name="status_news" id="status_news" class="form-control" {{(!in_array(3,[1,2])) ? '' : 'disabled'}}>
                                        <option value="3" {{ $tidings_request->status == 3 ? 'selected' : '' }}>Menunggu Konfirmasi</option>
                                        <option value="2" {{ $tidings_request->status == 2 ? 'selected' : '' }}>Ditolak</option>
                                        <option value="1" {{ $tidings_request->status == 1 ? 'selected' : '' }}>Diterima</option>
                                    </select>
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-group row d-none" id="reason-form">
                    ...
                    <div class="col-sm-10">
                        <textarea name="reason" id="reason" cols="30" rows="10" class="form-control" {{(!in_array(3,[1,2])) ? '' : 'disabled'}}>{{ $tidings_request->reason }}</textarea>
                    </div>
                </div>
                @else
                @endhasanyrole
                <div class="text-right">
                    <a href="{{ route('tidings-request.index') }}" class="btn btn-danger p-2 rounded">Kembali</a>
...
@section('script-footer')
    ...
    <script type="text/javascript">
        ...
        $('#forminput').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('tidings-request.update', $tidings_request->id) }}",
                type: "POST",
                processData: false,
                contentType: false,
                data: formData,
                ...
                success: (res) => {
                    if (res.success == true) {
                        window.location.href = "{{ route('tidings-request.index') }}"
                    } else {
                        alert(res.message);
                    }
                },
