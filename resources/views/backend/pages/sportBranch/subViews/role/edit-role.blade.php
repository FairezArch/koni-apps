    ...
    <div class="shadow-sm p-3 mb-5 bg-white rounded">
        ...
        <form id="forminput" class="forminput mt-3 mb-3">
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="role_name" name="role_name" aria-describedby="name" placeholder="Peran" value="{{ucfirst($role_permission->roles->name)}}">
                @error('name')
            ...
            <div class="col-xs-12 col-sm-12">
                <div class="row">
                    <table class="table">
                        <thead>
                            <tr>
                                <th rowspan="2" class="text-center border center" scope="col">Permission</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center" scope="row">
                                    <input type="checkbox" id="click" class="@error('permission') is-invalid @enderror" id="permission" name="permission" value="{{$permission->id}}" {{in_array($permission->id, $findPermission) ? 'checked="checked"' : ''}}>
                                </td>
                                @endif
                                @endforeach
                            </tr>
            ...
            <div class="text-right">
                <a href="{{route('sport-branch.role-permission.index', $sport_branch->id)}}" class="btn btn-danger p-2 rounded">Kembali</a>
...
<script type="text/javascript">
    ...
    $('#forminput').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{route('sport-branch.role-permission.update', [$sport_branch->id, $role_permission->id])}}",
            type: "POST",
            processData: false,
            contentType: false,
            data: formData, 
            ...
            success: function(res) {
                if(res.success == true){
                    window.location.href="{{route('sport-branch.role-permission.index', $sport_branch->id)}}"
                }else{
                    alert(res.message);
                }
            },
