                            ...
                            <div class="col-sm-10">
                                <select name="clubs_id" id="clubs_id" class="form-control">
                                    @foreach ($clubs as $club)
                                        <option value="{{ $club->id }}" {{ $lists->clubs_id == $club->id ? 'selected' : '' }} >{{ $club->club_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
...
@section('script-footer')
    ...
    <script type="text/javascript">
        ...
        $('#forminput').on('submit', function(e) {
            ...
            let trainer_id = $('#trainer_id').val(); // Sport ID
            ...
            $.ajax({
                url: "{!! url('sport-branch/"+sport_id+"/trainer/"+trainer_id+"') !!}",
                type: "POST",
                processData: false,
                contentType: false,
                data: formData,
