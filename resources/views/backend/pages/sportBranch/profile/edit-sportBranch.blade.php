        ...
        <div class="shadow-sm p-3 mb-5 bg-white rounded">
            ...
            <form id="forminput" class="forminput mt-3 mb-3" enctype="multipart/form-data">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="row">
                                ...
                                <div class="col-sm-8">
                                    <input type="text" name="users_idInfo" value="{{ $lists->users->name }}"
                            id="users_idInfo" class="form-control" disabled>
                                    <input type="text" name="users_id" value="{{ json_encode([$lists->id, $lists->sk_number]) }}"
                                    id="users_id" class="form-control" disabled hidden>
                                </div>
                            </div>
                        </div>
                        <div class="col-md-6">
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <div class="show-image fade-image-cust d-inline-block" id="show-image"
                                ...
                                <img src='{{ asset("storage/sport/".$lists->file_sport) }}'
                ...
                <div class="text-right">
                    <a href="{{ route('sport-branch.profile', $lists->id) }}" class="btn btn-danger p-2 rounded">Kembali</a>
    ...
    <script type="text/javascript">
        $(function() {
            if($('#users_id').length > 0){
                let splitSelect = JSON.parse($('#users_id').val());
        ...
        $('#forminput').on('submit', function(e) {
            ...
            $.ajax({
                url: "{{ route('sport-branch.profile.update', $lists->id) }}",
                type: "POST",
                processData: false,
                contentType: false,
                data: formData,
                ...
                success: function(res) {
                    if (res.success == true) {
                        window.location.href = "{{ route('sport-branch.profile', $lists->id) }}"
                    } else {
                        alert(res.message);
                    }
                },
