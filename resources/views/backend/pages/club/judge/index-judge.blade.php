...
<div class="container-fluid">
    <div class="wrapper-table p-3 bg-white rounded">
        <div class="w-100 mt-3 mb-3 text-right">
            @can('judge-create')
                <a class="btn btn-danger" href="{{route('club.judge.create', $club->id)}}">Tambah Wasit Juri</a>
            @endcan
        </div>
...
<script type="text/javascript">
  $(function () {
        ...
        columns: [
            ...
            {data: 'action', orderable: false, searchable: false, 
                render: function(dataField){ 
                    ...
                    return `@can('judge-edit')<a href="{{url('club/`+splitdata[1]+`/judge/`+splitdata[0]+`/edit')}}" class='edit m-1'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil" viewBox="0 0 16 16">
    ...
    $('body').on('click', '.delete-record', function(e){
        ...
        let id = $(this).data("id");
        let split  = id.split(',');
        let url = "{!! url('club/"+split[1]+"/judge/"+split[0]+"') !!}";
