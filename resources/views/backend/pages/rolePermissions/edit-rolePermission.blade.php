...
<div class="container-fluid">
    <div class="shadow-sm p-3 mb-5 bg-white rounded">
        ...
        <form action="{{route('rolepermission.update',$role->id)}}" method="POST" id="forminput" class="forminput mt-3 mb-3">
            @csrf
            @method('PUT')
            <div class="form-group">
                <label for="name">Role</label>
                <input type="text" class="form-control" id="role_name" name="role_name" aria-describedby="name" placeholder="Peran" value="{{ucfirst($role->name)}}">
                @error('name')
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center" scope="row">
                                    <input type="checkbox" id="click" class="@error('permission') is-invalid @enderror" id="permission" name="permission" value="{{$permission->id}}" {{in_array($permission->id, $rolePermissions) ? 'checked="checked"' : ''}}>
                                </td>
                                @endif
                                @endforeach
                            </tr>
...
<script type="text/javascript">
    ...
    $('#forminput').on('submit', function(e) {
        ...
        $.ajax({
            url: "{{route('rolepermission.update',$role->id)}}",
            type: "POST",
            processData: false,
            contentType: false,
            data: formData, 
